...
void	draw_set(t_vars *vars)
{
	...
	while (y < vars->sizey)
	{
		...
		while (x < vars->sizex)
		{
			...
			if (vars->type == calc_mandel)
				col = vars->type(a, b, a, b);
			else
				col = vars->type(a, b, vars->xconst, vars->yconst);
			col = color_set(col, vars->colshift);
